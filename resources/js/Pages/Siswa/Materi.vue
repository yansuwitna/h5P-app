...
<template>
    <AppLayout>
        <Head title="Katalog Modul Interaktif" />
        ...
        <div class="space-y-6">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white dark:bg-[#1c2434] p-5 sm:p-6 rounded-2xl border border-slate-200 dark:border-[#2e3a4b] shadow-sm">
                ...
                <span class="text-xs px-3 py-1.5 rounded-xl bg-slate-100 dark:bg-[#243044] text-slate-700 dark:text-slate-200 font-semibold border border-slate-200 dark:border-[#2e3a4b] self-start sm:self-auto">
                    {{ materi?.length ?? 0 }} Materi Tersedia
                </span>
            </div>

            <!-- Grid Katalog Materi -->
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                <div v-for="m in materi" :key="m.id" class="bg-white dark:bg-[#1c2434] rounded-2xl p-5 border border-slate-200 dark:border-[#2e3a4b] hover:border-brand-500 transition-all shadow-sm flex flex-col justify-between gap-4">
                    <div class="space-y-3">
                        <div class="flex items-center justify-between">
                            ...
                            <span v-if="getSkor(m.id)" class="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-semibold bg-amber-50 text-amber-700 dark:bg-amber-950/50 dark:text-amber-300 border border-amber-200">
                                Lencana {{ getSkor(m.id).lencana }} &bull; {{ getSkor(m.id).skor }}/{{ getSkor(m.id).skor_maksimal }}
                            </span>
                            <span v-else class="text-[11px] text-slate-400">Belum dikerjakan</span>
                        </div>
                    ...
                    <div class="pt-3 border-t border-slate-100 dark:border-[#2e3a4b] flex items-center justify-between">
                        ...
                        <a :href="`/materi/kerjakan/${m.id}`" class="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg text-xs font-semibold transition" :class="getSkor(m.id) ? 'bg-slate-100 hover:bg-slate-200 dark:bg-[#243044] dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200' : 'bg-brand-600 hover:bg-brand-700 text-white shadow-sm'">
                            <span>{{ getSkor(m.id) ? 'Ulangi Kuis' : 'Mulai Belajar' }}</span>
                ...
                <div v-if="!materi || materi.length === 0" class="col-span-full bg-white dark:bg-[#1c2434] rounded-2xl p-12 text-center text-slate-400 border border-slate-200 dark:border-[#2e3a4b]">
