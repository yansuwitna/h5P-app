...
const submitExcel = () => {
    formExcel.post('/admin/import-siswa', {
        onSuccess: () => {
            formExcel.reset();
        },
    });
        ...
        <div class="space-y-6">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white dark:bg-[#1c2434] p-5 sm:p-6 rounded-2xl border border-slate-200 dark:border-[#2e3a4b] shadow-sm">
                ...
                <div class="flex items-center gap-2">
                    <span class="text-xs px-3 py-1.5 rounded-xl bg-slate-100 dark:bg-[#243044] text-slate-700 dark:text-slate-200 font-semibold border border-slate-200 dark:border-[#2e3a4b]">
                        Total: {{ daftarSiswa?.length ?? 0 }} Siswa
                    </span>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-[#1c2434] rounded-2xl border border-slate-200 dark:border-[#2e3a4b] shadow-sm overflow-hidden">
                <div class="p-5 border-b border-slate-200 dark:border-[#2e3a4b] flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    ...
                    <span v-show="tab === 'excel'" class="text-xs text-slate-400 font-mono">Format kolom: Nama | NISN | Email | Password</span>
                </div>
                    ...
                    <form @submit.prevent="submitManual" class="space-y-3">
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            <div>
                                ...
                                <input v-model="formManual.nama" type="text" required placeholder="Contoh: Muhammad Rizky Pratama" class="w-full px-3.5 py-2 rounded-xl bg-white dark:bg-[#243044] border border-slate-200 dark:border-[#2e3a4b] text-xs">
                            </div>
                            <div>
                                ...
                                <input v-model="formManual.nisn" type="text" required placeholder="Contoh: 0081234567" class="w-full px-3.5 py-2 rounded-xl bg-white dark:bg-[#243044] border border-slate-200 dark:border-[#2e3a4b] text-xs font-mono">
                            </div>
                        </div>
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            <div>
                                <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Alamat Email (Opsional)</label>
                                <input v-model="formManual.email" type="email" placeholder="Contoh: rizky@siswa.sch.id" class="w-full px-3.5 py-2 rounded-xl bg-white dark:bg-[#243044] border border-slate-200 dark:border-[#2e3a4b] text-xs">
                            </div>
                            <div>
                                <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Kata Sandi (Minimal 6 Karakter)</label>
                                <input v-model="formManual.kata_sandi" type="password" required placeholder="Masukkan kata sandi siswa" class="w-full px-3.5 py-2 rounded-xl bg-white dark:bg-[#243044] border border-slate-200 dark:border-[#2e3a4b] text-xs">
                            </div>
                        </div>
                        <div class="flex justify-end pt-1">
                            <button type="submit" :disabled="formManual.processing" class="px-5 py-2.5 rounded-xl bg-brand-600 hover:bg-brand-700 text-white font-semibold text-xs transition shadow-sm">
                                {{ formManual.processing ? 'Menyimpan...' : 'Simpan Akun Siswa' }}
                            </button>
                        </div>
                    </form>
                </div>
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-left text-xs border-collapse">
                        ...
                        <tbody class="divide-y divide-slate-100 dark:divide-[#2e3a4b]">
                            <tr v-for="s in daftarSiswa" :key="s.id" class="hover:bg-slate-50/50 dark:hover:bg-[#243044]/40 transition">
                                ...
                                <td class="py-3.5 px-6 text-slate-400">{{ formatDate(s.created_at) }}</td>
                            </tr>
                            <tr v-if="!daftarSiswa || daftarSiswa.length === 0">
