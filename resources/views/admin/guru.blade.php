    ...
    <div class="space-y-6">
        ...
        <div class="bg-white dark:bg-[#1c2434] rounded-2xl border border-slate-200 dark:border-[#2e3a4b] shadow-sm overflow-hidden" x-data="{ tab: 'excel' }">
            <div class="p-5 border-b border-slate-200 dark:border-[#2e3a4b] flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                ...
                <span x-show="tab === 'excel'" class="text-xs text-slate-400 font-mono">Format kolom: Nama | NIK | Email | Password</span>
            </div>
            ...
            <div x-show="tab === 'excel'" class="p-5 bg-slate-50/50 dark:bg-[#1c2434]/40 border-b border-slate-200 dark:border-[#2e3a4b]">
                <form action="{{ route('admin.guru.import') }}" method="POST" enctype="multipart/form-data" class="flex flex-col sm:flex-row items-center gap-3">
                    @csrf
            ...
            <div x-show="tab === 'manual'" class="p-5 bg-slate-50/50 dark:bg-[#1c2434]/40 border-b border-slate-200 dark:border-[#2e3a4b]" style="display: none;">
                <form action="{{ route('admin.guru.manual') }}" method="POST" class="space-y-3">
                    @csrf
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div>
                            <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Nama Lengkap Guru</label>
                            <input type="text" name="nama" required placeholder="Contoh: Dra. Nurhayati, M.Pd" class="w-full px-3.5 py-2 rounded-xl bg-white dark:bg-[#243044] border border-slate-200 dark:border-[#2e3a4b] text-xs">
                        </div>
                        <div>
                            ...
                            <input type="text" name="nik" required placeholder="Contoh: 3201234567890001" class="w-full px-3.5 py-2 rounded-xl bg-white dark:bg-[#243044] border border-slate-200 dark:border-[#2e3a4b] text-xs font-mono">
                        </div>
                    </div>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div>
                            <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Alamat Email (Opsional)</label>
                            <input type="email" name="email" placeholder="Contoh: nurhayati@sekolah.sch.id" class="w-full px-3.5 py-2 rounded-xl bg-white dark:bg-[#243044] border border-slate-200 dark:border-[#2e3a4b] text-xs">
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Kata Sandi (Minimal 6 Karakter)</label>
                            <input type="password" name="kata_sandi" required placeholder="Masukkan kata sandi guru" class="w-full px-3.5 py-2 rounded-xl bg-white dark:bg-[#243044] border border-slate-200 dark:border-[#2e3a4b] text-xs">
                        </div>
                    </div>
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-[#2e3a4b]">
                        @forelse($daftarGuru as $g)
                            <tr class="hover:bg-slate-50/50 dark:hover:bg-[#243044]/40 transition">
                                ...
                                <td class="py-3.5 px-6 text-slate-400">{{ $g->created_at->format('d M Y, H:i') }}</td>
                            </tr>
                        @empty
                            <tr>
